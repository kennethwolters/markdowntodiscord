...
The 500-case pilot produced 470 agreements across 499 protocol-valid comparisons (94.2%), 29 disagreements, and one quarantined citation record. Both critics passed all 18 existing policy-gold controls. A fresh blinded adjudicator routed the 30 reviewed cases to 12 passes and 18 triage-only failures. Seven independently supportable specification/policy behaviors were promoted to deterministic fixtures; that pilot produced baseline v2 with 1,177 cases and 25 passing gold fixtures. The current replay adds the minimized unresolved-reference regression for 1,178 cases, 26 passing gold fixtures, and zero invariant failures. Historical pilot hashes and authority limitations remain recorded in `data/reports/critic-pilot-v1.json`.
...
The initial v2 run retained 104 of 120 cases and quarantined 16. Its immutable baseline converter passed 66/83 validation labels (79.5%) and 16/21 challenge labels (76.2%); aggregate evidence remains in `data/reports/semantic-eval-v2.json`. Subsequent validation-only development improved to 77/83 after policy-supported converter and scorer fixes. The six remaining failures are label conflicts involving unpinned warning aliases, Discord-visible escaping, or a safely downgraded URL—not accepted converter defects. They require fresh adjudication or quarantine.

The v2 challenge was consumed by the baseline release-candidate run and is not reused while tuning. `eval:report` refuses to combine validation and challenge scores from different converter hashes. A new challenge must use `converter-policy-v3.md`, which pins canonical warning names and semantic matching rules, and must pass the same privacy and independence gates before the next release claim. All model labels remain silver evidence; observed Discord, specification, policy, and independently human-reviewed labels retain separate authority.
